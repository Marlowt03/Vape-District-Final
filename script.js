...
(() => {
  ...
  // --- First-responder single-tap navigation (mobile) ---
  // Runs BEFORE other handlers. If a tapped link does not own a submenu, navigate immediately.
  nav.addEventListener('click', (e) => {
    ...
    const ownsPanel = !!(li && hasPanel(li));
    if (!ownsPanel) {
      e.preventDefault();
...
nav.addEventListener('click', (e) => {
  ...
  // Normalize: if an li claims it has a submenu but there is no panel, strip those classes
  nav.querySelectorAll('li.has-submenu, li.submenu').forEach(li => {
    const panel = li.querySelector(':scope > .dropdown-content, :scope > .submenu, :scope > .sub-dropdown');
    if (!panel) li.classList.remove('has-submenu', 'submenu');
  });
  // Also strip anchor-level .has-sub when its parent doesn't truly own a panel
  nav.querySelectorAll('li > a.has-sub').forEach(a => {
    const li = a.closest('li');
    const panel = li?.querySelector(':scope > .dropdown-content, :scope > .submenu, :scope > .sub-dropdown');
