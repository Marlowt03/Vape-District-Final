...
(() => {
  ...
  // --- Global single-tap navigation guard (mobile) ---
  // Ensures ANY real link inside the nav goes on first tap if it doesn't own a panel.
  nav.addEventListener('click', (e) => {
    if (!isMobile()) return;
    ...
    if (!href || href === '#') return;
    // Respect links meant to open new tabs
    ...
    if (!ownsPanel) {
      ...
      window.location.assign(href); // force navigation on first tap
    }
  }, true);
